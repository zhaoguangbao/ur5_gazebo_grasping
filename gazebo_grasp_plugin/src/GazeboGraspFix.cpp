...
class GazeboGraspFix::ObjectContactInfo
{
    ...
    std::vector<Vector3d> appliedForces;
...
void GazeboGraspFix::OnUpdate() {
    ...
    for (objIt=contPoints.begin(); objIt!=contPoints.end(); ++objIt)
    {
        ...
        for (lIt=objIt->second.begin(); lIt!=objIt->second.end(); ++lIt){
            ...
            Vector3d avgForce=collP.force/collP.sum;
    ...
    for (gripCntIt = this->gripCounts.begin(); gripCntIt != this->gripCounts.end(); ++gripCntIt){
        ...
        for (pointIt=initColls.begin(); pointIt!=initColls.end(); ++pointIt)
        {
            ...
            Vector3d relContactPos=cpInfo.pos/cpInfo.sum;
            // initial distance from link to object (relative to link)
            Vector3d relObjPos=cpInfo.objPos/cpInfo.sum;
           
            // get current world pose of object 
            Pose3d currObjWorldPose=cpInfo.collObj->GetLink()->WorldPose();

            // get world pose of link
            Pose3d currLinkWorldPose=cpInfo.collLink->GetLink()->WorldPose();

            // Get transform for currLinkWorldPose as matrix
            ignition::math::Matrix4d worldToLink=ignition::math::Matrix4d(currLinkWorldPose.Rot());
            ...
            Vector3d currContactWorldPose=_currContactWorldPose.Translation();
            ...
            Vector3d oldObjDist= relContactPos - relObjPos;
            ...
            Vector3d newObjDist= currContactWorldPose - currObjWorldPose.Pos(); // new distance from contact to object
...
double angularDistance(const Vector3d& _v1, const Vector3d& _v2) {
    Vector3d v1=_v1;        
    Vector3d v2=_v2;
    v1.Normalize();
    v2.Normalize();
...
bool GazeboGraspFix::checkGrip(const std::vector<Vector3d>& forces, float minAngleDiff, float lengthRatio){
    ...
    std::vector<Vector3d>::const_iterator it1, it2;
...
void GazeboGraspFix::OnContact(const ConstContactsPtr &_msg)
{
    ...
    for (int i = 0; i < _msg->contact_size(); ++i) {
        ...
        if ((collision1 && !collision1->IsStatic()) && (collision2 && !collision2->IsStatic()))
        {
            ...
            Pose3d linkWorldPose=linkCollision->GetLink()->WorldPose();
            ...
            Vector3d contactPosInLocal = contactInLocal.Translation();
            
            //std::cout<<"---------"<<std::endl;    
            //std::cout<<"CNT in loc: "<<contactPosInLocal.x<<","<<contactPosInLocal.y<<","<<contactPosInLocal.z<<std::endl;

            /*Vector3d sDiff=avgPos-linkWorldPose.pos;
            ...
            Vector3d lX=linkWorldPose.rot.GetXAxis();    
            Vector3d lY=linkWorldPose.rot.GetYAxis();    
            Vector3d lZ=linkWorldPose.rot.GetZAxis();    
            ...
            Pose3d objWorldPose = objCollision->GetLink()->WorldPose();
            ignition::math::Matrix4d worldToObj = ignition::math::Matrix4d(objWorldPose.Rot());
            worldToObj.SetTranslation(objWorldPose.Pos());

            ignition::math::Matrix4d objInLocal = worldToLinkInv * worldToObj;
            Vector3d objPosInLocal = objInLocal.Translation();
